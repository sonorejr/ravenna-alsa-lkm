...
int Create(TRTP_audio_stream* self, TRTP_stream_info* pRTP_stream_info, rtp_audio_stream_ops* pManager, TEtherTubeNetfilter* pEth_netfilter)
{
	...
	MTAL_DP("========================================\n");
	MTAL_DP("Create RTPStream NIC[%u] %s %s\n", pRTP_stream_info->m_uiIfPortId, pRTP_stream_info->m_bSource ? "Source" : "Sink", pRTP_stream_info->m_cName);
...
int Destroy(TRTP_audio_stream* self)
{
	...
	{
		MTAL_DP("========================================\n");
		MTAL_DP("Destroy RTPStream NIC[%u] %s %s\n", pRTP_stream_info->m_uiIfPortId, pRTP_stream_info->m_bSource ? "Source" : "Sink", pRTP_stream_info->m_cName);
	}
	else
	{
		return 0;
...
int ProcessRTPAudioPacket(TRTP_audio_stream* self, TRTPPacketBase* pRTPPacketBase)
{
	...
	uint32_t ui32Len1;
	uint32_t ui32Len2;
	uint64_t ui64LastAudioSampleReceivedSAC;
	bool bCopyRTPAudioToLivesIn;
...
#pragma message("TODO: check if the RTP packet is in time")
	...
	ui64LastAudioSampleReceivedSAC = ui64RTPSAC + ui32NbOfSamplesInThisPacket;

	bCopyRTPAudioToLivesIn = true;
	// ST2022-7
	pAttachedStream = self->m_tRTPStream.m_pAttachedStream;
	if (pAttachedStream)
	{
	...
	/*if (self->m_ui32RTPPacketCounter % 10000 == 0)
	{
		printk("[%u] RTP packet received %u copy lives in %s\n", pEth_netfilter->nic_id, self->m_ui32RTPPacketCounter, bCopyRTPAudioToLivesIn ? "yes" : "no");
	}*/
	...
	self->m_tRTPStream.m_ui64LastAudioSampleReceivedSAC = ui64LastAudioSampleReceivedSAC;
	return 1;
}
...
*/
	TRTP_stream* pAttachedStream = self->m_tRTPStream.m_pAttachedStream; // this the stream on second NIC
	if (pEth_netfilter->nic_id == 1 && pAttachedStream)
	{ // ST2022-7: the stream on NIC 0 is going to send packets on both NICS
		return 0;
	}
	...
	{
		uint32_t ui32NbOfSamplesInThisPacket = min(pRTP_stream_info->m_ui32MaxSamplesPerPacket, ui32NbOfSampleRemaining);
...
int SendRTPAudioPacket(TRTP_audio_stream* self, const uint64_t ui64CurrentSAC, const uint32_t ui32Offset, const uint32_t ui32NbOfSamplesInThisPacket, const unsigned short usOutgoingSeqNum)
{
	...
	if (!IsLinkUp(pEth_netfilter))
	{
		//DEBUG_TRACE(("NIC[%u] is down\n", pEth_netfilter->nic_id));
		return 0;
	}

	ulMinPacketSize = sizeof(TRTPPacketBase) + GetNbOfLivesOut(self) * pRTP_stream_info->m_byWordLength * ui32NbOfSamplesInThisPacket; // RTP header + (ui32NbOfSamplesInThisPacket) samples for all channels

	MTAL_RtTraceEvent(RTTRACEEVENT_RTP_OUT + pEth_netfilter->nic_id, (PVOID)(RT_TRACE_EVENT_SIGNAL_START), 0);

	if (!AcquireTransmitPacket(pEth_netfilter, &pHandle, &pvPacket, &ulPacketSize) || ulPacketSize < ulMinPacketSize)
	{
		MTAL_RtTraceEvent(RTTRACEEVENT_RTP_OUT + pEth_netfilter->nic_id, (PVOID)(RT_TRACE_EVENT_SIGNAL_STOP), 0);
	...
	pTRTPPacket = (TRTPPacketBase*)pvPacket;
	...
	if (ui32Len1 > 0)
	{
		...
		MTAL_RtTraceEvent(RTTRACEEVENT_RTP_OUT + pEth_netfilter->nic_id, (PVOID)(RT_TRACE_EVENT_SIGNAL_START | RT_TRACE_EVENT_COLOR_BLUE), 0);
	...
	if (ui32Len2 > 0)
	{
		...
		MTAL_RtTraceEvent(RTTRACEEVENT_RTP_OUT + pEth_netfilter->nic_id, (PVOID)(RT_TRACE_EVENT_SIGNAL_START | RT_TRACE_EVENT_COLOR_TURQUOISE), 0);
	...
	MTAL_RtTraceEvent(RTTRACEEVENT_RTP_OUT + pEth_netfilter->nic_id, (PVOID)(RT_TRACE_EVENT_SIGNAL_START | RT_TRACE_EVENT_COLOR_YELLOW), 0);
	// Update IP's header
	...
	pTRTPPacket->RTPHeader.usSeqNum = MTAL_SWAP16(usOutgoingSeqNum);
	...
	//MTAL_DP("RTP out ui32TimeStamp 0x%x BufferSAC 0x%llx\n", (uint32_t)ui64CurrentSAC, ui64BufferSAC);
	...
	/*pTRTPPacket->UDPHeader.usCheckSum	= MTAL_SWAP16(MTAL_ComputeUDPChecksum(&pTRTPPacket->UDPHeader,
	(uint16_t)ui32PacketSize - sizeof(TEthernetHeader)  - sizeof(TIPV4Header),
	(uint16_t*)&pTRTPPacket->IPV4Header.ui32SrcIP,
	(uint16_t*)&pTRTPPacket->IPV4Header.ui32DestIP));
	*/

	MTAL_RtTraceEvent(RTTRACEEVENT_RTP_OUT + pEth_netfilter->nic_id, (PVOID)(RT_TRACE_EVENT_SIGNAL_STOP), 0);

	MTAL_RtTraceEvent(RTTRACEEVENT_RTP_OUT + pEth_netfilter->nic_id, (PVOID)(RT_TRACE_EVENT_SIGNAL_START | RT_TRACE_EVENT_COLOR_PURPLE), 0);
	...
	{
		//MTAL_DP("This RTP Ouput packet is not contiguous with the previous one SAC = %d should be %d, last size was: %d\n", MTAL_SWAP32(pTRTPPacket->RTPHeader.ui32Timestamp), m_ui32LastRTPSAC + m_ui32LastRTPLengthInSamples, ui32NbOfSamplesInThisPacket);
	}
...
void PrepareBufferLives(TRTP_audio_stream* self)
{
	...
	bool bGlobalLivesInMuted = false;
	int64_t delta;
	TEtherTubeNetfilter* pEth_netfilter = self->m_tRTPStream.m_pEth_netfilter;
	TRTP_stream_info* pRTP_stream_info = &self->m_tRTPStream.m_RTP_stream_info;
	rtp_audio_stream_ops* pManager = self->m_pManager;
	...
	{
		// sink status (used by m_StreamStatus)
		bool bLivesInMuted = IsLivesInMustBeMuted(self);


		// ST2022-7
		// global sink status (used by m_StreamStatus)
		bGlobalLivesInMuted = bLivesInMuted;
		pAttachedStream = self->m_tRTPStream.m_pAttachedStream;
		if (pAttachedStream)
		{ // Mute only when both streams must be muted
			bGlobalLivesInMuted &= IsLivesInMustBeMuted((TRTP_audio_stream*)pAttachedStream);
		}

		// check if the current (i.e. at GlobalSAC()) audio frame of the sink must be muted
		{
			if (bGlobalLivesInMuted)
			{
	...
	if (!pRTP_stream_info->m_bSource)
	{
		...
		if (self->m_ui32StreamStatusResetCounter != self->m_ui32StreamStatusLastResetCounter)
		{
			...
			self->m_StreamStatus.u.bit_fields.sink_all_muted = bGlobalLivesInMuted ? 1 : 0;
		}
		else
		{ // update error only
			if (bSinkIsReceiving)
			...
			if (bGlobalLivesInMuted)
				self->m_StreamStatus.u.bit_fields.sink_all_muted = 1;
			if (bWrongRTPSeqId)
...
{
	self->m_byNICId = pEth_netfilter->nic_id;
	self->m_bActive = 0;

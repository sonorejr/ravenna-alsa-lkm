                ...
                for (i = 0; i < MAX_SOURCE_STREAMS * self->m_usNumberOfNICS * 2; i++)
                {
                    if(&self->m_apRTPSourceStreams[i].m_RTPAudioStream == (TRTP_audio_stream*)(size_t)hRTPStream)
                    {
                        if (IsActive(&self->m_apRTPSourceStreams[i]))
                        {
							...
							printk("CRTP_streams_manager::RemoveRTPStream: remove stream %p on port Id = %d\n", 
									&self->m_apRTPSourceStreams[i], self->m_apRTPSourceStreams[i].m_byNICId);
                            ret = 1;
                            break;
                        }
	...
	if (pRTPStream)
	{
		...
		for (us = 0; us < usNumberOfRTPStreams; us++)
		{
			...
			{				
				...
				printk("CRTP_streams_manager::attached_stream: found ST2022-7 %s stream %u", pRTPStream->m_RTP_stream_info.m_bSource ? "source" : "sink", pRTPStream->m_RTP_stream_info.m_uiId);
				break;
			}
		}
	}
